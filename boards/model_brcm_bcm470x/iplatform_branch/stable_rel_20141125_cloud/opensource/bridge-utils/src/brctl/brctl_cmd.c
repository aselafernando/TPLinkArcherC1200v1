...
static int br_cmd_setifvlan(int argc, char *const* argv)
{
	const char *brname;
	int err;
	...
	if(argc-- > 0)
	{
		...
		err = br_set_interface_vlan(brname, ifname, vlanid, vlanpos);
		...
		default:
			fprintf(stderr, "can't set %s vlan to bridge %s: %s\n",
				ifname, brname, strerror(err));
		}
		return 1;
	}
...
static int br_cmd_setiftag(int argc, char *const* argv)
{
	const char *brname;
	const char *vlantag;
	int err;
	...
	if(argc-- > 0)
	{
		...
		err = br_set_interface_tag(brname, ifname, vlantag);
...
static int br_cmd_showvlan(int argc, char *const* argv)
{
	if(argc == 2)
		br_show_vlan(argv[1]);
...
static const struct command commands[] = {
	...
	{ 2, "setifvlan", br_cmd_setifvlan,
	  "<bridge> <device> [vlanid] [pos]\tset vlan id to interface" },
	{ 3, "setiftag", br_cmd_setiftag,
	  "<bridge> <device> {tag|untag}\tset tag to interface" },
	...
	{ 1, "showvlan", br_cmd_showvlan,
	  "[ <bridge> ]\t\tshow bridges vlan info" },
	{ 2, "stp", br_cmd_stp,
