...
int control_finish (struct tunnel *t, struct call *c)
{
    ...
    {
		#ifdef TP_WEB_LOG
		if (msgtypes[c->msgtype] != NULL)
			if(memcmp(msgtypes[c->msgtype], "Hello", 5) != 0)
				log_to_web("rcvd %s", msgtypes[c->msgtype]);
		#else
        l2tp_log (LOG_DEBUG,
