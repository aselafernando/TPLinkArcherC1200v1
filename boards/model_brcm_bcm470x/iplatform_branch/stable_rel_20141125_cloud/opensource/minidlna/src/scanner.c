...
int directory_exist_in_db(const char* path)
{
	char** results;
	int rows;
	char cmd[1000];
	...
	//目录
	sprintf(cmd,"SELECT PATH from DETAILS where PATH = \"%s\" and MIME is NULL ",path);
	sql_get_table(db, cmd, &results, &rows, NULL);
...
static int file_exist_in_db(sqlite3* db,const char* path)
{
	char** results;
	int rows;
	int ret;
	...
	//媒体文件
	sprintf(cmd,"SELECT PATH from DETAILS where PATH = \"%s\" and MIME is not NULL ",path);
	sql_get_table(db, cmd, &results, &rows, NULL);
	sqlite3_free_table(results);
	...
	//playlist
	sprintf(cmd,"SELECT PATH from PLAYLISTS where PATH = \"%s\"",path);
	sql_get_table(db, cmd, &results, &rows, NULL);
	sqlite3_free_table(results);
