 ...
 */
void
sendBeaconMessage(struct sockaddr_in * client, int len, int broadcast)
{
	char * mesg;
	int mesg_len;
	...
	sendto(sbeacon, mesg, mesg_len, 0, (struct sockaddr*)client, len);
	free(mesg);
}
...
void ProcessTiVoBeacon(int *s)
{
	...
	n = recvfrom(sock, bufr, sizeof(bufr), 0,
	...
	if( rcvBeaconMessage(bufr) )
		sendBeaconMessage(sock, &sendername, len_r, 0);
}
...
int tivo_init()
{
    ...
    DPRINTF(E_WARN, L_TIVO, "TiVo support is enabled.\n");
    ...
    if(sbeacon < 0)
    {
        DPRINTF(E_FATAL, L_TIVO, "Failed to open sockets for sending Tivo beacon notify "
            "messages. EXITING\n");
    }

    evt = event_malloc(sbeacon);
    if (!evt)
    {
        DPRINTF(E_FATAL, L_TIVO, "Failed to malloc event_struct. EXITING\n");
    }
    evt->read.data = &evt->sock;
    evt->read.handler = ProcessTiVoBeacon;
    ret = event_ctl(evt, EPOLL_CTL_ADD, EPOLLIN);
    if (ret != 0)
    {
        DPRINTF(E_FATAL, L_TIVO, "Failed to event_ctl sbeacon. EXITING\n");
    }
