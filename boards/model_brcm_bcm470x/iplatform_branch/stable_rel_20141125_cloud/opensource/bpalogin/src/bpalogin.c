...
int mainloop(struct session * s)
{
	...
	/* added by yangxv for disable redial in bpalogin process */
	s->shutdown = 1;
	/* end add */
	...
	/* The BPA server use the fixed udp port 5050 as the destination port of heartbeats,
	 *so the port of listensock cannot allocate dynamically. 09Mar2007, Tan Rifei*/
