...
static int find_module(char *filename)
{
	...
	* if it is a link... */
	...
	/* 
	**	Orig Sentence is "module_dir = concat_path_file(CONFIG_DEFAULT_MODULES_DIR, myuname.release);"
	**	Now there is "iplatform" dir in CONFIG_DEFAULT_MODULES_DIR, and some ko files in it, so we just scan
	**	CONFIG_DEFAULT_MODULES_DIR path.
	@@	By TengFei, 14/09/02.
	*/
	module_dir = strdup(CONFIG_DEFAULT_MODULES_DIR);
	if (!module_dir)
	{
		return ENOMEM;
	}
