 ...
 *
 * $Id: channelcapacity.js 544222 2015-03-26 10:48:42Z spalanga $
 */
...
{
	clearAllTimers(TIMER_CHANNEL_CAPACITY | TIMER_ASSOCSTA);
...
function DisplayStaApInfo(islayoutreq, selectedBand) {
	...
	$.ajax({
		...
		success:function(result) {
			...
			macarr = drawAP(array, ''); /* Pass the empty mac address as there is no selection here */
			macarr = [];
		}
	});
	array = [];
	...
	if (islayoutreq == true) { // Only if layout is required
		...
		for (i = 0; i < chnldata[1].length; i++) {
			if (chnldata[1][i].Heading == 'ChannelMap') { // Only for channel map (channel distribution) graph
				...
				/* Hide the 80 MHz bandwidth graph in 2.4 GHz band */
				if (selectedBand.band == 2 && (chnldata[1][i].Bandwidth == 80)) {
					var tmphidden;
					tmphidden = "#" + id + i;
