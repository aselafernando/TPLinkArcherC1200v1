 ...
 *
 * $Id: vis_wl.c 568904 2015-07-06 11:04:01Z $
 */
...
static networks_list_t *dump_networks(char *buf);
static void dump_bss_info(wl_bss_info_t *bi, int idx, networks_list_t *networks_listin);
...
wl_dump_networks(void *wl)
{
	...
	if (ret == 0) {
		tmp_network = dump_networks(dump_buf);
	}
...
dump_networks(char *network_buf)
{
	...
	for (i = 0; i < list->count; i++, bi = (wl_bss_info_t*)((int8*)bi + dtoh32(bi->length))) {
		dump_bss_info(bi, i, tmp_network);
	}
...
/* get the Max rate */
static void
get_max_rate(wl_bss_info_t *bi, bool ntypefound, int *has_b, int *has_g, int *outmaxrate)
{
...
static void
dump_bss_info(wl_bss_info_t *bi, int idx, networks_list_t *networks_listin)
{
	...
	get_max_rate(bi, ntypefound, &has_b, &has_g, &maxrate);
...
/* Gets the DUT info */
static int
get_dut_info_from_bss(wl_bss_info_t *bi, dut_info_t *dut_info)
{
	char chspec_str[CHANSPEC_STR_LEN];
	wl_bss_info_107_t *old_bi;
	...
	if ((ret = wlu_get(wl, WLC_GET_BSSID, &bssid, ETHER_ADDR_LEN)) == 0) {
		...
		*(uint32*)g_vis_cmdoutbuf = htod32(WLC_IOCTL_MAXLEN);
		...
		if (dtoh32(bi->version) == WL_BSS_INFO_VERSION ||
			...
			get_dut_info_from_bss(bi, dut_info);
