 ...
 *
 * $Id: matrics.js 544222 2015-03-26 10:48:42Z spalanga $
 */
...
function plotthecounters(islayoutreq, selectedBand) {
	...
	if (mac != 0) {
		pullPacketQueueStats(false, selectedBand);
	}
	if (selectedBand.mac == $("#bandselectcntrl").val())
...
function drawthecounter(array, index, islayoutreq, selectedBand) {
	...
	$.ajax({
		...
		success:function(result) {
			...
			drawTimeSeriesStats(array, data[0][0], "#"+data[0][0].Placeholderid, true);
		}
	});
}
...
function onSelectingAssociatedSTA()
{
	if (staclkarr[staclkarr.length - 1] != mac) {
		...
		pullPacketQueueStats(true, selectedBand);
	}
}
...
/* Gets packet queue stats */
function pullPacketQueueStats(islayoutreq, selectedBand) {
	...
	if (staval == -1) {
		...
		index = 0;
		fetchPacketQueueStats(array, index, islayoutreq, mac, selectedBand);
	}
}

/* Requests packet queue stats from server */
function fetchPacketQueueStats(array, index, islayoutreq, mac, selectedBand) {
	var data;
	...
	$.ajax({
		...
		success:function(result) {
			...
			for (var i = 0; i < data[0].length; i++) {
				...
				drawTimeSeriesStats(array, data[0][i], "#"+data[0][i].Placeholderid, true);
			}
			createCustomizedGraphTable(array, islayoutreq);
		}
	});
}

/* Creates layout for customized garph of packet queue stats */
function createCustomizedGraphTable(array, islayoutreq) {
	if (islayoutreq == true) {
		var elementadd = '';
		var element = '';
		var increment = 0;
		var length = packetQueueData[0][1].Total + packetQueueData[0][2].Total;
		var nCount = 0;
		var nCountInRow = 7;

		elementadd += '<table><tr>';
		for (var i = 0; i < packetQueueData[0].length; i++) {
			for (var j = 0; j < packetQueueData[0][i].Total; j++) {
				if ((nCount % nCountInRow) == 0)
					elementadd += '</tr><tr>';
				elementadd += '<td><input type="checkbox" id="' + "checkbox" + increment++ + '" ';
				elementadd += 'style="width:25px;height:15px;margin-left:30px;" ';
				elementadd += 'value=' + '"'+ packetQueueData[0][i].BarHeading[j] + ';"';
				elementadd += '>' + packetQueueData[0][i].BarHeading[j] +'</td>';
				nCount++;
			}
		}
		elementadd += '</tr></table>';
		element = '<td style="width:35%;"><table class="packetqueuegraphtable">';
		...
		element += '<tbody><tr><td>'+ elementadd + '<tr><td>\
	...
	fillCustomizedGraphWithData(array);
}

/* Fills packet queue stats customized graph */
function fillCustomizedGraphWithData(array) {
	// packetQueueData object consists of all the other objects of the packet queue statistics
	// create a one temporary object which contains the members of all the objects together in one object from the packetQueueData object
	...
	var concatTotal = '';
	// Deep copy
	var packetQueueTemp = jQuery.extend(true, {}, packetQueueData[0][0]);
	var packetQueueTemp1 = jQuery.extend(true, {}, packetQueueData[0][0]);

	packetQueueTemp1.YValue = [];
	packetQueueTemp1.BarHeading = [];
	packetQueueTemp1.Total = [];
	drawTimeSeriesStatsForCustom(array, packetQueueTemp1,"#"+"customplaceholder");

	for (var i = 0; i < packetQueueData[0].length; i++) {
		tmpLength += packetQueueData[0][i].Total;
		concatYValue = concatYValue.concat(packetQueueData[0][i].YValue);
		concatBarHeading = concatBarHeading.concat(packetQueueData[0][i].BarHeading);
		concatTotal += packetQueueData[0][i].Total;
	}
	...
	//customgraphtimer.push(setTimeout(function(){fillCustomizedGraphWithData(array);},500));
}
...
function drawTimeSeriesStats(array, data, placeholder, iscolorlabel) {
