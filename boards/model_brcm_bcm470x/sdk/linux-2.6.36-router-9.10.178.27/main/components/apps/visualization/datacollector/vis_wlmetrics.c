 ...
 *
 * $Id: vis_wlmetrics.c 555336 2015-05-08 09:52:04Z $
 */
...
#define MCSTCNT_LT40_FROM_CNTBUF(cntbuf)                                \
	bcm_get_data_from_xtlv_buf(((wl_cnt_info_t *)cntbuf)->data,     \
		((wl_cnt_info_t *)cntbuf)->datalen,                     \
		WL_CNT_XTLV_LT40_UCODE_V1, NULL,                        \
		BCM_XTLV_OPTION_ALIGN32)

#define MCSTCNT_GE40_FROM_CNTBUF(cntbuf)                                \
	bcm_get_data_from_xtlv_buf(((wl_cnt_info_t *)cntbuf)->data,     \
		((wl_cnt_info_t *)cntbuf)->datalen,                     \
...
wl_counters(void *wl, counters_t *countersout, counters_t *oldcountersout)
{
	...
	err = wlu_iovar_get(wl, "counters", cntbuf, WL_CNTBUF_MAX_SIZE);
	if (err < 0) {
	...
	err = wl_cntbuf_to_xtlv_format(NULL, cntbuf, WL_CNTBUF_MAX_SIZE, corerev);
	if (err < 0) {
