...
int BCMFASTPATH_HOST tcp_gro_complete(struct sk_buff *skb)
{
	...
	/* set flags to tell dev_gso_segment to use skb_segment, but not skb_tcp_segment,
 	 * in tcp_tso_segment, BRCM add skb_tcp_segment routine, which has some bugs for 
 	 * fowarding skbs (gro received).
 	*/ 
 	skb->tcpf_nf = 1;
