...
#define TPUSB
#define TPUSB_MTD_OFFSET  0x00FB0000
#define TPUSB_MTD_SIZE    0x10000

#define TPLOG
#define TPLOG_MTD_OFFSET  0x00FC0000
#define TPLOG_MTD_SIZE    0x20000
...
#ifdef TPUSB
    /* Setup usb-config MTD partition */
    bcm947xx_flash_parts[nparts].name = "usb-config";
	bcm947xx_flash_parts[nparts].size = ROUNDUP(TPUSB_MTD_SIZE, mtd->erasesize);
	bcm947xx_flash_parts[nparts].offset = TPUSB_MTD_OFFSET;
	nparts++;
#endif

#ifdef TPLOG
    /* Setup log MTD partition */
    bcm947xx_flash_parts[nparts].name = "log";
	bcm947xx_flash_parts[nparts].size = ROUNDUP(TPLOG_MTD_SIZE, mtd->erasesize);
	bcm947xx_flash_parts[nparts].offset = TPLOG_MTD_OFFSET;
	nparts++;
#endif
