...
#else
		brc->vid = vlan_dev_vlan_id(rxdev) | vlan_dev_vlan_egress_prio(rxdev, 0);
#endif
...
#endif
{
	...
	if (fdb) {
		...
		if (!is_local && (source->state == BR_STATE_FORWARDING) 
			&& !strcmp(source->br->dev->name,"br-lan") )
...
#endif
{
	...
	if (likely(fdb)) {
		...
		if (unlikely(fdb->is_local)) {
			 ...
			 * the same bridge.
			 */
			if (source->state == BR_STATE_FORWARDING && !strcmp(br->dev->name,"br-lan"))
				br_brc_add((unsigned char *)addr, source->dev, skb);
#endif /* HNDCTF */
